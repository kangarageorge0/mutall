...
async function create_crown():Promise<void>{
    //
    //Get the names
    const sql:string = `select name from user`;
    //
...
async function create_body(){
    ...
    body_data.forEach(row=>create_body_row(body_elem));
    //
...
function create_body_row(body:HTMLTableSectionElement){
    //
    //Create a body row element
    const tr =<HTMLTableRowElement> create_element('tr', body);
    //
...
function create_footer_row(
    ...
    const fcell = create_sticky_cell(tr, 'left');
    //
    //Set the column span of the footer cell to 2
    fcell.setAttribute('colSpan', '2');
    fcell.textContent = summary;
    //
...
async function fill_footer_cells(
    ...
    //
    //Fill the crest region cells. Its teh first cell and has its cell 
...
function fill_footer_crumb_cells(
    footer:HTMLTableSectionElement,
    event:{event:string, summaries:string}
){
    //Use the get_element methods that does not crash if the ement name is not 
    //valid
