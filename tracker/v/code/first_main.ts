...
type page_selection = {[key in factor]:lib.basic_value};

//Jpint is really not a factpr as it were, but is a field of page selector
type factor= 'school'|'year'|'class'|'exam'|'stream'|'student'|'date'|'joint';


//Tabulate exam results .......View exam results in a given sitting
abstract class empty_exam_results extends outlook.baby<void>{
    ...
    public body_data?:lib.Ifuel;   
     //
    //The row that determines the horizontal size of a tables
    public header?:HTMLTableRowElement;
    
    //The page data
    public page_data?:Array<page_selection>;
    //
    public page_selector_element?:HTMLSelectElement;
    //
    //All the factors
    public factors:Array<factor>=['school', 'year', 'class', 'exam', 'stream', 'student', 'date'];
    //
    //The page factors
    public page_factors:Array<factor>;
    //
    //The base query
    public base_query?:string;
      
    //The row factors are all the facttprs without the page factorss
    public  get row_factors():Array<factor>{
        //
    ...
    constructor(mother:main, page_factors:Array<factor>){
    ...
    //the factors
    async fill_filter_options():Promise <void>{
        //
        //Get the base quety
        const base = 
        `
    ...
    async show_page():Promise<void>{
        ...
        const items:Array<factor> = this.page_factors;
        //
    ...
    async get_page_data():Promise<Array<page_selection>>{
        //
    ...
    async fill_paginator_selector():Promise <void>{ 
        //
        //Get the page data
        this.page_data = await this.get_page_data();    
        //
        //Get the page selector element
        this.page_selector_element = <HTMLSelectElement>this.get_element('selector');
        //
    ...
    async set_filter_values():Promise<void>{
       //
        //Get the current selection index from the sitting selector
        const current_selection:number = this.page_selector_element!.selectedIndex;
        //
        //Use the selection index to get the corresponding page data row
        const row:page_selection = this.page_data![current_selection];
        //
        //For each page item....
    ...
    get_page_condition(page:{[key in factor]:lib.basic_value}):string{
        //
    ...
    async get_body_data():Promise <lib.Ifuel>{
        //
    ...
    set_score_cells(raw_values:string, tr:HTMLTableRowElement):void{
        ...
        const subjects:Array<{subject:string, value:number}>
       ...
       for(const subject of subjects){
           ...
           for(const name of  ['value'] as Array<keyof typeof subject>){
               //
